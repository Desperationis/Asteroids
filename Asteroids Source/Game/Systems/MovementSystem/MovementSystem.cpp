...
void MovementSystem::updatePhysics(const EntityID& id, const EntityMask& mask)
{
	...
	if ((mask & COMPONENT_WALLLOOP) == COMPONENT_WALLLOOP) {
		if (currentPosition.x + currentTexture.dest.w < 0) {
			currentPosition.x = CONSOLE::WINDOW_WIDTH;
		}
		if (currentPosition.x > CONSOLE::WINDOW_WIDTH) {
			currentPosition.x = 0 - currentTexture.dest.w;
		}

		if (currentPosition.y > CONSOLE::WINDOW_HEIGHT) {
			currentPosition.y = -(currentTexture.dest.h);
		}
		if (currentPosition.y + currentTexture.dest.h < 0) {
			currentPosition.y = CONSOLE::WINDOW_HEIGHT;
		}
