...
void MovementSystem::updatePhysics(const EntityID& id, const EntityMask& mask)
{
	...
	if ((mask & COMPONENT_WALLLOOP) == COMPONENT_WALLLOOP) {
		if (currentRect.right < 0 + ENTITY::BORDER_DISTANCE) {
			currentPosition.x = CONSOLE::WINDOW_WIDTH - ((currentRect.right - currentRect.left) / 2.0f) - ENTITY::BORDER_DISTANCE;
		}
		if (currentRect.left > CONSOLE::WINDOW_WIDTH - ENTITY::BORDER_DISTANCE) {
			currentPosition.x = -(currentRect.right - currentRect.left) / 1.0f + ENTITY::BORDER_DISTANCE;
		}
