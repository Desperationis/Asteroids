...
void LevelSystem::loadMeteorData(int level, const char* loading, MeteorSystem::METEOR size, float speed) {
	...
	for (int i = 0; i < levels[tmp.str().c_str()][loading]; i++) {
		...
		switch (location) {
		case 0: //left
			data.x = 10000; data.y = rand() % 1080; // 10000 so it teleports to left
			break;
		case 1: // top
			data.x = rand() % 1920, data.y = 10000; // 10000 so it teleports to top
			break;
		case 2: // right
			data.x = -10000, data.y = rand() % 1080; //-10000 so it telports to the right
			break;
		case 3: //bottom
			data.x = rand() % 1920, data.y = -10000; //-10000 so it teleports to the bottom
			break;
		}
		data.angle = rand() % 361;
